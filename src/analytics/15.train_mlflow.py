'''
Código otimizado com MLFlow, que é um gerenciador de experimentos onde podemos comparar os desempenhos
de diferentes modelos usando métricas pre estabelecidas

1. Instalar MLflow (pip install)
2. No terminal, acessar a pasta raid do projeto e iniciar o MLFlow : [mlflow server]
3. Abrir MLFlow e inicar um novo experimento e capturar o Id de Sessão
4. Voltando ao código, importar a bibliioteca vincular à sessão e projeto MLFlow
    > mlflow.set_tracking_uri
    > mlflow.set_experiment
5. Deslocar o bloco de código incluindo o treinamento dos modelos e criação das métricas
    > with mlflow.run
    > Caso necessário, pode-se gerar gráficos como artefatos e adiciona-los ao log do MLFlow
'''
#%%
import pandas as pd
...
# Vinculando projeto ao experimento
...
# Vamos criar um dicionario para registrar os parametros do nosso modelo e tambem usar no log
# do MLFlow, preparando as listas de hiperparametros pro GridSearch

dic_modelos = {
    "DecisionTree" : {"model" : tree.DecisionTreeClassifier , 
                      "params" : {"random_state" : 42 , 
                                  "min_samples_leaf" : 10} ,
                      "grid_search" : { "min_samples_leaf" : [3, 5, 10 , 20, 30, 50]}                        
    } ,
    "RandomForest" : {"model" : ensemble.RandomForestClassifier , 
                      "params" : {"random_state" : 42 , 
                                  "min_samples_leaf" : 30 ,
                                  "n_estimators" : 150 ,
                                  "n_jobs" : -1} ,
    ...
    "AdaBoost" : {"model" : ensemble.AdaBoostClassifier , 
                  "params" : {"random_state" : 42 , 
                              "n_estimators" : 150 ,
                              "learning_rate" : 0.1} ,
                  "grid_search" : {"n_estimators" : [50, 100, 150, 200, 500, 1000] ,
                                   "learning_rate" : [0.001, 0.01, 0.05, 0.1, 0.2, 0.5, 0.9, 0.99]}
    }
}
# %%

# Agora deslocamos todo o bloco de fit e testes para dentro do mlflow, tipando as tags e nomes
# de runs que acharmos necessaria a documentação

with mlflow.start_run(run_name="model_fiel_v3") as run:

    # O autolog do sklearn ja faz a captura de várias caracteristicas do modelo, mas podemos
    # definir capturas individuais
    mlflow.sklearn.autolog()
    
    # Podemos definir algumas tags para facilitar a documentação
    # Usamos aqui o método SET_TAGS (plural). o SET_TAG (singular) recebe pares key-value
    run_tags = {
    ...
    # Agora invocamos o dicionario para montar o modelo, desempacotando o dicionario com as keywords
    nome_modelo = "AdaBoost"
    model_config = dic_modelos[nome_modelo]
    model = model_config['model'](**model_config['params'])

    # Vamos adicionar um GridSearch para tunar os hiperparametros, recebendo MODEL
    grid_params = model_config['grid_search']
    ...
    # MODIFY - IMPUTATION (Pipeline, com a saida da ultima etapa alterada de MODEL para GRID)
    ...
    # Registrar as features para leitura em produção
    mlflow.log_dict({"features" : features} , "raw_features.json")

    model_pipeline.fit(X_train , y_train)

    # Vamos transformar o PREDICT, PREDICT_PROBA e METRICAS em saidas de uma função

    def run_model(X , y):
        """Função para rodar os predicts e armazenar em variaveis"""
        
        X_ = X
        y_ = y
        
        pred = model_pipeline.predict(X_)
        proba = model_pipeline.predict_proba(X_)

        metrics_ = {
            "acc" : metrics.accuracy_score(y_true=y_ , y_pred=pred) ,
            "auc" : metrics.roc_auc_score(y_true=y_ , y_score=proba[:,1])
        }

        # Retona FPR , TPR , THRESHOLD
        roc_curve = metrics.roc_curve(y_true=y_ , y_score=proba[:,1])

        return pred , proba , metrics_ , roc_curve


    X_oot = df_oot[features]
    y_oot = df_oot[target]

    y_train_pred , y_train_proba , train_metrics , train_roc_curve = run_model(X=X_train , y=y_train)
    y_test_pred , y_test_proba , test_metrics , test_roc_curve = run_model(X=X_test , y=y_test)
    y_oot_pred , y_oot_proba , oot_metrics , oot_roc_curve = run_model(X=X_oot , y=y_oot)

    # Logar as métricas, alterando os prefixos para identificar as variaveis, desempacotando os
    # pares key-value
    mlflow.log_metrics({
        **{ f"train_{chave}":valor for chave,valor in train_metrics.items() } , 
        **{ f"test_{chave}":valor for chave,valor in test_metrics.items() } , 
        **{ f"oot_{chave}":valor for chave,valor in oot_metrics.items() }
        })

    # Grafico ROC sobrepondo ambas as series
    fig, ax = plt.subplots()
    
    ax.set_title(f"Curva ROC do Modelo : {nome_modelo}" , loc='left')
    
    ax.plot(train_roc_curve[0], train_roc_curve[1]) # ROC Treino
    ax.plot(test_roc_curve[0], test_roc_curve[1]) # ROC Teste
    ax.plot(oot_roc_curve[0], oot_roc_curve[1]) # ROC OOT
    ax.plot([0,1] , [0,1] , '--' , color='black')
    
    ax.legend([
        f"Treino : {train_metrics['auc']:.4f}" ,
        f"Teste : {test_metrics['auc']:.4f}" ,
        f"OOT : {oot_metrics['auc']:.4f}"
    ])

    ax.grid(visible=True, linewidth=0.3 , color='gray', linestyle='-.')

    nome_grafico = 'curva_roc.png'
    plt.savefig(nome_grafico)
    mlflow.log_artifact(nome_grafico)

    nome_grafico = 'lift curve Treino'
    skplot.metrics.plot_lift_curve(y_true=y_train , y_probas=y_train_proba, title=nome_grafico)
    plt.savefig(f"{nome_grafico}.png")
    mlflow.log_artifact(f"{nome_grafico}.png")

    nome_grafico = 'lift curve Teste'
    skplot.metrics.plot_lift_curve(y_true=y_train , y_probas=y_test_proba, title=nome_grafico)
    plt.savefig(f"{nome_grafico}.png")
    mlflow.log_artifact(f"{nome_grafico}.png")
    
    nome_grafico = 'lift curve OOT'
    skplot.metrics.plot_lift_curve(y_true=y_train , y_probas=y_oot_proba, title=nome_grafico)
    plt.savefig(f"{nome_grafico}.png")
    mlflow.log_artifact(f"{nome_grafico}.png")
    ...
    # Capturar as features importances para logar no MLFlow
